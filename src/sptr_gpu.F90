...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   real(kind=b8) :: rk2,uiuj,term,dterm,fact
   ...
   if(taskid.eq.0)write(6,*)"Enter Sptr_gpu"
   rtime1=MPI_WTIME()
   ...
   !$OMP TARGET DATA MAP(to:uny) MAP(to:bkx2, bky2, bkz2, mask, ssq, beta_min) &
   !$OMP MAP(from:eijky,dijky,der2y,sijky,gijky,grijky,ekky,xrijy,yrijy,zrijy)
   ...
   !$OMP bkz2,nc,mask,uny,ssq,nyp2,nzp2,beta_min,tfact) REDUCTION(+:eijky, &
         ...
         do xp=1,xisz
            ...
            do i=1,3+nc
               do j=i,3+nc
                  ...
                  uiuj=real(uny(xp,y,zp,i)*conjg(uny(xp,y,zp,j)))
                  ...
                  xrijy(x,ij)=xrijy(x,ij)+uiuj

                  if(y.eq.1) then
                     yrijy(y,ij)=yrijy(y,ij)+uiuj+(tfact(x)-1)*uiuj
                  else if(y.lt.nyhp) then
                     yrijy(y,ij)=yrijy(y,ij)+uiuj
                  else
                     iy=nyp2-y
                     yrijy(iy,ij)=yrijy(iy,ij)+(tfact(x)-1)*uiuj
                  endif

                  if(z.eq.1) then
                     zrijy(z,ij)=zrijy(z,ij)+uiuj+(tfact(x)-1)*uiuj
                  else if(z.lt.nzhp) then
                     zrijy(z,ij)=zrijy(z,ij)+uiuj
                  else
                     iz=nzp2-z
                     zrijy(iz,ij)=zrijy(iz,ij)+(tfact(x)-1)*uiuj
                  endif
               end do ! j
            end do ! i
   ...
   rtime2=MPI_WTIME()-rtime2
   if (taskid.eq.0) write(6,"(' Sptr: Parallel (secs) :',1p,e12.4)") rtime2
   rtime4=rtime2
   ...
   !call MPI_ALLREDUCE (eijky,eijk,mxyz*ncp,mpireal,MPI_SUM,&
   !   MPI_COMM_WORLD,mpierr)
   !call MPI_ALLREDUCE (dijky,dijk,mxyz*ncp,mpireal,MPI_SUM,&
   !   MPI_COMM_WORLD,mpierr)
   !call MPI_ALLREDUCE (sijky,sijk,mxyz*ncp,mpireal,MPI_SUM,&
   !   MPI_COMM_WORLD,mpierr)
   !call MPI_ALLREDUCE (gijky,gijk,mxyz*ncp,mpireal,MPI_SUM,&
   !   MPI_COMM_WORLD,mpierr)
   !call MPI_ALLREDUCE (ekky,ekk,mxyz,mpireal,MPI_SUM,&
   !   MPI_COMM_WORLD,mpierr) 
   !call MPI_REDUCE (der2y,der2ij,ncp,mpireal,MPI_SUM,0,&
   !   MPI_COMM_WORLD,mpierr)
   !call MPI_ALLREDUCE (grijky,grijk,mxyz*ncp*3,mpireal,MPI_SUM,&
   !   MPI_COMM_WORLD,mpierr)

   !call MPI_REDUCE (xrijy,xrij,nxhp*ncp,mpireal,MPI_SUM,0,&
   !   MPI_COMM_WORLD,mpierr)
   !call MPI_REDUCE (yrijy,yrij,nyhp*ncp,mpireal,MPI_SUM,0,&
   !   MPI_COMM_WORLD,mpierr)
   !call MPI_REDUCE (zrijy,zrij,nzhp*ncp,mpireal,MPI_SUM,0,&
   !   MPI_COMM_WORLD,mpierr)
   ...
   if (taskid.eq.0) write(6,"(' Sptr: Pack1 (secs) :',1p,e12.4)") rtime3

   rtime3=MPI_WTIME()
   call MPI_BARRIER(MPI_COMM_WORLD,mpierr)
   rtime3=MPI_WTIME()-rtime3
   if (taskid.eq.0) write(6,"(' Sptr: Barrier1 (secs) :',i6,1p,2e12.4)") &
      taskid,rtime3,rtime4
   ...
   call MPI_REDUCE (sndbuf1,rcvbuf1,ncount,mpireal,MPI_SUM,0,&
      MPI_COMM_WORLD,mpierr)
   rtime3=MPI_WTIME()-rtime3
   if (taskid.eq.0) write(6,"(' Sptr: Reduce1 (secs) :',1p,e12.4)") rtime3
   ...
   rtime3=MPI_WTIME()-rtime3
   if (taskid.eq.0) write(6,"(' Sptr: Unpack1 (secs) :',1p,e12.4)") rtime3
   ...
   rtime3=MPI_WTIME()-rtime3
   if (taskid.eq.0) write(6,"(' Sptr: Pack2 (secs) :',1p,e12.4)") rtime3

   rtime3=MPI_WTIME()
   call MPI_BARRIER(MPI_COMM_WORLD,mpierr)
   rtime3=MPI_WTIME()-rtime3
   if (taskid.eq.0) write(6,"(' Sptr: Barrier2 (secs) :',1p,e12.4)") rtime3
   ...
   call MPI_REDUCE (sndbuf1,rcvbuf1,ncount,mpireal,MPI_SUM,0,&
      MPI_COMM_WORLD,mpierr)
   rtime3=MPI_WTIME()-rtime3
   if (taskid.eq.0) write(6,"(' Sptr: Reduce2 (secs) :',1p,e12.4)") rtime3
   ...
   deallocate (sndbuf1,rcvbuf1)
   rtime3=MPI_WTIME()-rtime3
   if (taskid.eq.0) write(6,"(' Sptr: Unpack2 (secs) :',1p,e12.4)") rtime3
